...
- **取得日付の明示**: 株価・財務・決算予定などのデータを引用する際は、必ず取得日付を明記する。「現在」「直近」「最新」といった曖昧な表現は禁止
   - **ラグの自発的注意喚起**: データ取得日付と本日の乖離が1ヶ月以上ある場合、回答内で必ず注意喚起する（例: 「※株価データは2.5ヶ月前のものです。直近の値動きは web_search で補完してください」）
   - **ソース別の独立明示**: 財務（EDINET、最新）と株価（J-Quants、3ヶ月遅延）を組み合わせる際は、それぞれの取得日付を独立して明示する
   - 詳細は「既知の制約・注意点」セクションを参照
